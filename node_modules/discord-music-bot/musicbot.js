...
var commands = [
...

  {
	...
	//Spencer's custom commands ($)

	{
    command: "$target",
		description: "Set a target for functions that require one",
		parameters: ["Username of target"],
		execute: function (message, params) {

			if (params.length < 1 || params.length > 1)
			{
				text_channel.sendMessage("Please target a single user -- \"$target [targetUser]\"");
				return;
			}
			targetUser = params[1];

			text_channel.sendMessage(targetUser + " has been targeted. May God have mercy on your soul.");

		}
	},
	{
    command: "$ben",
		description: "Ben's Requested Command",
		parameters: [],
		execute: function (message, params) {

			if (targetUser === null)
			{
				text_channel.sendMessage("Please target a user -- \"$target [targetUser]\"");
				return;
			}
			else if (message.author !== Ben) {
				text_channel.sendMessage("Only Ben can determine the success of action");
				return;
			}

			var dieResult = (Math.floor(Math.random() * 20)) + 1;

			text_channel.sendMessage("Ben rolled a " + dieResult);

			if (dieResult > 3) {
				text_channel.sendMessage(targetUser + " performs action");
			}
			else {
				text_channel.sendMessage("Congratulations, " + targetUser + ", you failed to perform the most basic of actions");
			}
	...
	{
    command: "$jeff",
		description: "Jeff's Requested Command",
		parameters: [],
		execute: function (message, params) {

			if(!stopped) {
				stopMusic (message, params);
			}

			//text_channel.sendMessage("Jeff Words");

			var audio_stream = ytdl("https://www.youtube.com/watch?v=MrTsuvykUZk");
			voice_handler = voice_connection.playStream(audio_stream);

			if(stopped && !is_queue_empty()) {
				resumeMusic (message, params)
			}
	...
	{
    command: "$bender",
		description: "Bender's Requested Command",
		parameters: [],
		execute: function (message, params) {

			if(!stopped) {
				stopMusic ("stop", params);
			}

			text_channel.sendMessage("Bite my Shiny Metal Ass");

			var audio_stream = ytdl("https://www.youtube.com/watch?v=-pRnQYS0S5c");
			voice_handler = voice_connection.playStream(audio_stream);

			if(stopped && !is_queue_empty()) {
				resumeMusic ("resume", params)
			}
	...
	{
    command: "$woody",
		description: "Woodard's Requested Command",
		parameters: [],
		execute: function (message, params) {

			var audio_stream = ytdl("https://www.youtube.com/watch?v=KnsiZOJjfUg");
			voice_handler = voice_connection.playStream(audio_stream);
	...
	{
    command: "$spencer",
		description: "Spencer's Custom Command",
		parameters: [],
		execute: function spencer(message, params) {
			//Add a random super ridiculous song to the current queue
			...
			*/
			...
			var audio_stream = ytdl("https://www.youtube.com/watch?v=upre46qXW_Y#t=31s");
			voice_handler = voice_connection.playStream(audio_stream);
...
function customCommmandQueueSkip()
{
